...
object LyricsUtils {
    ...
    fun parseLyrics(lyrics: String): List<LyricsEntry> {
        ...
        val lines = decodedLyrics.lines()
            .filter { line ->
                ...
                if (trimmed.startsWith("[offset:")) {
                    val offsetStr = trimmed.substringAfter("[offset:").substringBefore("]")
                    offsetMs = offsetStr.toLongOrNull() ?: 0L
                    false
                } else {
                    trimmed.isNotBlank()
                }
        ...
        val parsedLines = if (isRichSync) {
            parseRichSyncLyrics(lines)
        } else {
            parseStandardLyrics(lines)
        }
        
        if (offsetMs == 0L) return parsedLines
        
        // Apply offset (positive offset means lyrics appear sooner, so subtract from time)
        return parsedLines.map { entry ->
            entry.copy(
                time = maxOf(0L, entry.time - offsetMs),
                words = entry.words?.map { word ->
                    word.copy(
                        startTime = maxOf(0.0, word.startTime - (offsetMs / 1000.0)),
                        endTime = maxOf(0.0, word.endTime - (offsetMs / 1000.0))
                    )
                }
            )
        }
